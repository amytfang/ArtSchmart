...
class Painter {
  ...
  draw(pos) {
    ...
    if (this.brushStyle === "pointillism"){
      ctx.beginPath();
      ctx.arc(pos[0], pos[1], this.brushSize - 5, 0, 2 * Math.PI);
      ctx.fillStyle = "white";
      ctx.fill();
      const spacing = this.canvasClarity * 4;
  ...
  drawCircle(pixel) {
    ...
    ctx.beginPath();
    ctx.arc(posX, posY, this.canvasClarity * 2 - 1, 0, 2 * Math.PI);
  ...
  drawCurve(pixel) {
    ...
    const ctx = this.painting.ctx;
    const width = Math.random() * this.canvasClarity * 6;
    const height = Math.random() * this.canvasClarity * 6;
